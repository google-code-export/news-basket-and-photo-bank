...
class Manage_user extends Controller {
	...
	function edit_user_process() {
		if ($this -> session -> userdata('user_level') == 'administrator' && $this -> session -> userdata('login') == TRUE) {
			...
			$user = array('name' => $this -> input -> post('name'), 'id_source' => $this -> input -> post('publisher'), 'password' => dohash($password), 'email' => $this -> input -> post('email'), 'phone' => $this -> input -> post('phone'), 'user_level' => $this -> input -> post('user-level'));
	...
	function checkUsernameAvailability() {
		...
		if ($this -> Users_model -> checkUsernameAvailability($username) == TRUE) {
			echo "no";
			// username ini terpakai
		} else if (strlen($username) == 0) {
			echo "no";
			// username ini terpakai
		} else {
			echo "yes";
			// username ini tidak terpakai
		}
