...
class Dashboard extends Controller {
	...
	function home() {
		$admin['page_title'] = 'Dashboard | Publisher Newsbasket';
		$admin['username']   = $this->session->userdata['username'];
		$admin['main_view']  = 'user/dashboard_view';
		...
		$this->load->view('user/template', $admin);
	}
	...
	function edit_profile($id_user) { 
		$data_user['page_title']	 	= 'Edit User | Publisher News Basket';
		$data_user['main_view'] 	 	= 'user/detail/editor_detail_view';
		$data_user['edit_user_form']	= 'user/form/edit_editor_form';
		$link_manage_user				= site_url('user/dashboard');
		$link_detail_user				= site_url('user/dashboard/myprofile');
		$data_user['breadcrumb']		= "<a href='$link_manage_user' style='color: white;'>Dashboard</a> > 
											   ...
											   > Edit Profile";
		$data_user['form_action_edit']	= site_url('user/dashboard/edit_profile_process').'/'.$id_user;
	...
	function edit_profile_process() {
		if ($this->session->userdata('user_level') == 'publisher' && $this->session->userdata('login') == TRUE) {
			...
			redirect('user/dashboard/myprofile');
		}
		else {
