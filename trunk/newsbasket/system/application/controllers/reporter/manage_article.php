...
class Manage_article extends Controller {
	...
	function load_article() {
		...
		// Membuat pagination			
		$config['base_url']    		= site_url('reporter/manage_article/load_article/');
	...
	function detail_article($id_article) {
		...
		$data_article['main_view'] 	 		= 'reporter/detail/article_detail_view';
		$data_article['article_property']	= 'reporter/detail/article_detail_property';
		$link_manage_article				= site_url('reporter/manage_article');
		$data_article['breadcrumb']			= "<a href='$link_manage_article' style='color: white;'>Manage Article</a> > Article Detail";
		$data_article['form_action_edit']	= site_url('reporter/manage_article/edit_article').'/'.$id_article;
	...
	function add_article() {
		...
		$data_article['main_view']			= 'reporter/form/add_article_form';
		$link_manage_article				= site_url('reporter/manage_article');
		$data_article['breadcrumb']			= "<a href='$link_manage_article' style='color: white;'>Manage Article</a>
											   > Add  New Article";		
		$data_article['form_action_add']	= site_url('reporter/manage_article/add_article_process');
		...
		if ((($this->session->userdata('user_level') == 'editor') || ($this->session->userdata('user_level') == 'reporter')) && $this->session->userdata('login') == TRUE) {
			...
			if ($this->session->userdata('user_level') == 'reporter') {
				$activity_log = array(
			...
			redirect('reporter/manage_article');
		}
		else {
	...
	function edit_article($id_article) { 
		...
		$data_article['main_view'] 	 		= 'reporter/detail/article_detail_view';
		$data_article['edit_article_form']	= 'reporter/form/edit_article_form';
		$link_manage_article				= site_url('reporter/manage_article');
		$link_detail_article				= site_url('reporter/manage_article/detail_article').'/'.$id_article;
		$data_article['breadcrumb']			= "<a href='$link_manage_article' style='color: white;'>Manage Article</a>
											   > Edit Article";
		$data_article['form_action_edit']	= site_url('reporter/manage_article/edit_article_process').'/'.$id_article;
		...
		if ($this->session->userdata('login') == TRUE && $this->session->userdata('user_level') == 'reporter' && $this->Article_model->checkLockArticle($id_article) == TRUE) {
			//lock article
		...
		?>
			...
			document.location='<?php echo site_url('reporter/manage_article')?>';
			</script>
			<?php
		}
	}
	...
	function edit_article_process() {
		if ((($this->session->userdata('user_level') == 'editor') || ($this->session->userdata('user_level') == 'reporter')) && $this->session->userdata('login') == TRUE) {
			...
			$this->session->set_flashdata('message_success', $message);
			redirect('reporter/manage_article/detail_article'.'/'.$id_article);
		}
		else {
...
/* Location: ./system/application/controllers/reporter/manage_article.php */