<h2>
	<a href="<?php echo site_url('editor/manage_user');?>" style="color: white;">Manage User</a> > Search User
	...
	<table id="user" class="tablesorter">
		...
		if (!empty($result) && $count != 0) {
			...
			foreach ($result as $column) {
				$deleteLink = anchor(
					'editor/manage_user/delete_user/'.$column->id_user,
					'<button>Delete</button>',
					array('class'=>'btn-delete', 'onclick'=>"return confirm('Are you sure want to delete this user?')")
				);
				$detailLink = anchor(
					'editor/manage_user/detail_user/'.$column->id_user,
