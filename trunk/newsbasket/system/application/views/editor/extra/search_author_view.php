<h2>
	<a href="<?php echo site_url('editor/manage_author');?>" style="color: white;">Manage Author</a> > Search Author
	...
	<table id="author" class="tablesorter">
		...
		if (!empty($result) && $count != 0) {
			...
			foreach ($result as $column) {
				$deleteLink = anchor(
					'editor/manage_author/delete_author/'.$column->id_author,
					'<button>Delete</button>',
					array('class'=>'btn-delete', 'onclick'=>"return confirm('Are you sure want to delete this author?')")
				);
				$editLink = anchor(
					'editor/manage_author/edit_author/'.$column->id_author,
					'<button>Edit</button>',
